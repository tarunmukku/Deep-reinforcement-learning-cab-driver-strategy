...
class CabDriver():
    ...
    def __init__(self):
        ...
        self.state_space = [(p,time,day) for p in range(m) for time in range(t) for day in range(d)]
    ...
    def next_state_func(self, state, action, Time_matrix):
        ...
        curr_loc = state[0]
        pickup_loc = self.action_space[action][0]
        drop_loc = self.action_space[action][1]
        curr_time = state[1]
        curr_day = state[2]
        """
         3 Scenarios: 
